...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance = self.distance + (((self.x - x) ** 2 + (self.y - y) ** 2)**0.5)
        self.x = x
        self.y = y
    ...
    def get_distance_from(self, point):
        self.distance_from = (((point.y - self.y)**2) + ((point.x - self.x)**2))**0.5
        return self.distance_from

    def get_distance_from_start(self):
        self.distance_from_start = (((self.y - self.init_y) ** 2) + ((self.x - self.init_x) ** 2)) ** 0.5
